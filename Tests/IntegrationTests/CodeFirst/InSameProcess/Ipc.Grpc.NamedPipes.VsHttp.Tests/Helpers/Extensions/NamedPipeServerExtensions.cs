...
namespace Ipc.Grpc.NamedPipes.VsHttp.Tests.Helpers.Extensions
{
    ...
    {
        public static void Bind<TService>(this GrpcDotNetNamedPipes.NamedPipeServer namedPipeServer, TService service) where TService : class
        {
        ...
        private class NamedPipeServerBinder : ServerBinder
        {
            private static ServiceBinderBase GetServiceBinderBase(object state)
            {
                return state switch
                {
                    GrpcDotNetNamedPipes.NamedPipeServer server => server.ServiceBinder,
